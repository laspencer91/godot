...
- **Smoke:** `bash workspace-editor-planning/smoke/run_smoke.sh` — must exit 0 with **zero** error-class lines. It copies the fixture to temp; **never** run the editor directly on the committed `smoke/` fixture (it pollutes `project.godot` / writes `.godot/`). The harness only exercises the MAIN-view path; the manual checks listed per step are therefore mandatory, not optional.
...
5. **`ScreenHostDocument`** (M5): ONE special document (new `EditorDocument::TYPE_SCREEN_HOST`) whose `DocumentView` hosts `main_screen_vbox` itself. Focusing Game/AssetLib/any third-party main screen = `reveal(screen_host_doc)` + the legacy `_select_index` dance *inside* it. This keeps `get_editor_main_screen()->get_control()` returning the live `main_screen_vbox` (D11) and reuses the entire existing make_visible machinery inside one tab.
...
- **Keep green:** the internal `tab_container` path still exists (old tabs keep working until S7); external-editor setting unchanged; `EditorHelp` opens still internal until S6b.
...
### M5.1 — ScreenHostDocument: singleton screens in a workspace tab

- **Intent:** implement seam #5.
- **Files:** `editor/editor_document.h` (add `TYPE_SCREEN_HOST`); `editor/editor_main_screen.{h,cpp}` (ctor `:320-333`, `focus_editor`); `editor/gui/document_view.cpp`.
- **Do:**
  - `EditorMainScreen` owns one `ScreenHostDocument` (a plain `EditorDocument` subclass, path `"screens://"`, no world).
  - `DocumentView` for `TYPE_SCREEN_HOST` reparents `main_screen_vbox` into itself; on view teardown/tab close, reparent back to a hidden compat holder child of `EditorMainScreen` (so `get_control()` at `editor_main_screen.cpp:259-261` always returns a live `main_screen_vbox` — D11 contract, byte-identical).
  - `focus_editor(name)`: after resolving the plugin, `reveal(screen_host_doc)` first, then `_select_index` (the make_visible dance runs inside the tab). Tab title follows the selected screen's plugin name. The ctor keeps the current default (root pane wraps `main_screen_vbox`) until first reveal moves it — pre-reveal behavior identical to today.
...
- **Verify:** build + smoke; manual: press AssetLib strip button → screen-host tab appears/focuses showing AssetLib; switch to Game → same tab retitles; close the tab → strip button re-summons it.
- **Commit:** `G2 M5.1: singleton screens (Game/AssetLib/third-party) live in a screen-host workspace tab`
...
- **Files:** `editor/editor_node.cpp` (the `add_main_plugin`/plugin-registration block — grep `add_main_plugin`; register CanvasItemEditorPlugin/Node3DEditorPlugin/ScriptEditorPlugin WITHOUT a strip entry: split `add_main_plugin` into strip registration vs plugin bookkeeping, or simply skip the strip add for these three and keep them as ordinary plugins); `editor/editor_main_screen.cpp` (`_notification(NOTIFICATION_READY)` default-select logic `:43-64` → default = focus the root tab host; ctor `:320-333` → root pane hosts a `TabbedDocumentHost` by default, `main_screen_vbox` parks in the hidden compat holder from M5.1); `editor/editor_node.cpp:7841-7861` (feature-profile gating: 2D/3D/Script profile flags now gate view-kind minting — 3D disabled ⇒ `DocumentView` never mints a 3D surface (falls back to 2D), Script disabled ⇒ `ScriptEditor::edit` external/no-op path — keep the existing profile checks' observable messages).
...
**S:** ☐ scripts and class docs open as workspace tabs via `reveal` ☐ debugger goto / attach-script / find-in-files / save-all / history all workspace-native ☐ `script_list` and internal tab hosting deleted ☐ shared script chrome follows the focused script tab ☐ members-overview deferral recorded ☐ pane split/close UX exists ☐ 45 `ScriptEditor::get_singleton()` callers un-edited and working ☐ smoke green; `/simplify` run; ledger row for `script_editor_plugin` added.
