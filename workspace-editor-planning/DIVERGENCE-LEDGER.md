...
| `editor/scene/canvas_item_editor_plugin.{cpp,h}` | SHIM | LOW-MED | The v1 2D shim: `scene_view_container` (a `SubViewportContainer` displaying the active doc's `scene_root`), `tree_signals_connected` reparent-tolerance flag, and display-active-doc plumbing. This whole surface is **slated for rewrite at Step ⑤** (CanvasView2D extraction), so treat conflicts here as throwaway — re-apply minimally, don't invest. |
