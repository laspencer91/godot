...
| `editor/scene/canvas_item_editor_plugin.{cpp,h}` | STRUCTURAL | **HIGH** | The 2D services/view split (Step ⑤b.4). `CanvasItemEditor` is the 2D services singleton + a new instanceable `CanvasItemEditorView` (both in this file), mirroring the Node3DEditor split. Landed: display stack + pan/zoom (⑤b.4a), overlay (⑤b.4b), input/drag (⑤b.4c), and document-binding (⑤b.4d — panes mint a view via `create_view_bound_to()`, each with its OWN `view_viewport` bound to the document's World2D; `CanvasView2D` deleted). Remaining: `_display_scene_root` shim retirement (⑤c). High churn on both sides of a rebase; re-apply the *view ownership* intent. The old SHIM surface (`scene_view_container` + `_display_scene_root`) is retired at ⑤c.2. |
