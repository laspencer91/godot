...
| `editor/editor_node.{cpp,h}` | STRUCTURAL | **HIGH** | The multi-document spine. Added `documents_holder` (persistent in-tree parent keeping every open doc's `scene_root` live), `register_document_context`, `_display_scene_root` (parks the deactivated doc's SubViewport back under `documents_holder` instead of detaching), `_activate_scene_views`, and `get_scene_root()` delegating to the active document. `editor_node.cpp` is one of upstream's most-churned files. `_display_scene_root`/`_activate_scene_views` are SCAFFOLDING (retire when DocumentView owns activation, Step ④+) but the doc-context registration is STRUCTURAL. |
...
| `editor/scene/canvas_item_editor_plugin.{cpp,h}` | STRUCTURAL | **HIGH** | The 2D services/view split (Step ⑤b.4, in progress). `CanvasItemEditor` is becoming the 2D services singleton + a new instanceable `CanvasItemEditorView` (both in this file), mirroring the Node3DEditor split. As of ⑤b.4a the display stack + pan/zoom moved to the view; overlay (⑤b.4b), input/drag (⑤b.4c), document-binding (⑤b.4d), and the `_display_scene_root` shim retirement (⑤c) follow. High churn on both sides of a rebase; re-apply the *view ownership* intent. The old SHIM surface (`scene_view_container` + `_display_scene_root`) is retired at ⑤c.2. |
